...
sub _build_default_headers {
    ...
    $headers->{'X-On-Behalf-Of'} = $self->on_behalf_of if $self->has_on_behalf_of;
    $headers;
}
...
sub _set_authorization_header {
    ...
    if ($self->has_http_token) {
        ...
        $auth_header = 'Basic '.encode_base64(join(':', $self->username, $self->password), '');
    }
...
sub object {
    my ($self) = @_;
    WWW::Zammad::Resource->new(resource => 'object_manager_attributes', client => $self);
}

sub online_notification {
    my ($self) = @_;
    WWW::Zammad::Resource->new(resource => 'online_notifications', client => $self);
}
...
sub ticket_article {
    my ($self) = @_;
    WWW::Zammad::Resource->new(resource => 'ticket_articles', client => $self);
}

sub ticket_attachment {
    my ($self) = @_;
    WWW::Zammad::Resource->new(resource => 'ticket_attachment', client => $self);
}

sub ticket_priority {
    my ($self) = @_;
    WWW::Zammad::Resource->new(resource => 'ticket_priorities', client => $self);
}
